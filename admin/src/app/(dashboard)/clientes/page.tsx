...
import { mockClientes, type ClienteNivel, type ClienteEstado } from '@/lib/mock-data';
...
function ScoreBadge({ score }: { score: number }) {
  const color = score >= 80 ? 'text-emerald-700' : score >= 60 ? 'text-amber-700' : 'text-red-600';
...
export default function ClientesPage() {
  ...
  const clientes = useMemo(() => {
    ...
    if (q) {
      list = list.filter((c) => c.nombre.toLowerCase().includes(q) || c.dni.includes(q));
    }
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        ...
        <div className="flex gap-2">
          ...
          >
            ...
            {(['PREMIUM', 'REGULAR', 'NUEVO', 'OBSERVADO', 'BLOQUEADO'] as ClienteNivel[]).map((n) => (
      ...
      <div className="rounded-lg border border-gray-200 bg-white shadow-sm overflow-hidden">
        <div className="border-b border-gray-100 px-5 py-3">
          <p className="text-xs text-gray-500">{clientes.length} cliente{clientes.length !== 1 ? 's' : ''}</p>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead className="bg-gray-50">
              <tr>
                {['Nombre', 'DNI', 'Teléfono', 'Score', 'Nivel', 'Servicios', 'Cancelaciones/mes', 'Estado'].map((h) => (
            ...
            <tbody className="divide-y divide-gray-50">
              {clientes.map((c) => (
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <span className={cn('inline-flex rounded-full px-2.5 py-0.5 text-[11px] font-semibold', nivelStyles[c.nivel])}>
                      {c.nivel}
                    </span>
                  </td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3">
                    <span className={cn('text-sm font-medium', c.cancelaciones_mes >= 3 ? 'text-red-600' : c.cancelaciones_mes >= 1 ? 'text-amber-600' : 'text-gray-600')}>
                      {c.cancelaciones_mes}
                    </span>
                  </td>
                  <td className="px-4 py-3">
                    <span className={cn('inline-flex rounded-full px-2.5 py-0.5 text-[11px] font-semibold', estadoStyles[c.estado])}>
                      {c.estado}
                    </span>
                  </td>
                </tr>
