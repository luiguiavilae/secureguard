...
function PayoutsTable({ initialPayouts }: { initialPayouts: MockPayoutPendiente[] }) {
  ...
  const columns = [
    ...
    payoutHelper.accessor('retencion_ir', {
      ...
      cell: (info) => (
        <span className="text-sm text-red-600">-{formatSoles(info.getValue())}</span>
      ),
    }),
    payoutHelper.accessor('monto_neto', {
      header: 'Monto neto',
      cell: (info) => (
        <span className="text-sm font-semibold text-emerald-700">{formatSoles(info.getValue())}</span>
      ),
    }),
      ...
      <div className="flex items-center justify-between border-b border-gray-100 px-5 py-3">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-0.5">
            {payouts.length} agentes con saldo pendiente
          </p>
        </div>
        <Button
          size="sm"
...
function TransaccionesTable({ txns }: { txns: MockTransaccion[] }) {
  ...
  const columns = [
    txnHelper.accessor('fecha', {
      header: 'Fecha',
      cell: (info) => (
        <span className="text-xs text-gray-500 tabular-nums" suppressHydrationWarning>
    ...
    txnHelper.accessor('metodo', {
      ...
      cell: (info) => (
        <span className="text-xs text-gray-500">
          {metodoLabels[info.getValue()] ?? info.getValue()}
        </span>
      ),
    }),
        ...
        return (
          <span
            className={cn(
              ...
              estadoStyles[e] ?? 'bg-gray-100 text-gray-600',
            )}
          >
            {e}
          </span>
    ...
    <div className="rounded-lg border border-gray-200 bg-white shadow-sm overflow-hidden">
      <div className="flex flex-col gap-3 border-b border-gray-100 px-5 py-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex gap-2">
          <select
            ...
            onChange={(e) =>
              table.getColumn('metodo')?.setFilterValue(e.target.value || undefined)
            }
          >
          ...
          <select
            ...
            onChange={(e) =>
              table.getColumn('estado')?.setFilterValue(e.target.value || undefined)
            }
          >
      ...
      <div className="flex items-center justify-between border-t border-gray-100 px-5 py-3">
        <p className="text-xs text-gray-500">
          ...
          {Math.max(table.getPageCount(), 1)} · {table.getFilteredRowModel().rows.length} registros
        </p>
        <div className="flex gap-2">
          <Button
            variant="outline"
  ...
  return (
    <div
      className={cn(
        ...
        isAlerta ? 'border-red-300' : 'border-gray-200',
      )}
    >
      <div
        className={cn(
          'flex items-center justify-between px-5 py-3 border-b',
          isAlerta ? 'border-red-200 bg-red-50' : 'border-gray-100',
        )}
      >
        ...
        <div className="mb-4 flex items-end gap-2">
          <span
            className={cn(
              ...
              isAlerta ? 'text-red-600' : 'text-gray-900',
            )}
          >
            {formatSoles(balance)}
          </span>
            ...
            >
              ...
              <span
                className={cn(
                  ...
                  m.monto > 0 ? 'text-emerald-700' : 'text-red-600',
                )}
              >
                {m.monto > 0 ? '+' : ''}
                {formatSoles(Math.abs(m.monto))}
              </span>
            </div>
          ))}
        </div>
...
interface Props {
  ...
  payouts: MockPayoutPendiente[];
}

export default function FinanzasClient({ transacciones, payouts }: Props) {
  const [periodo, setPeriodo] = useState<Periodo>('mes');

  const txnsFiltradas = useMemo(
    () => filterByPeriodo(transacciones, periodo),
    [transacciones, periodo],
  );
  const gmv = useMemo(
    () =>
      txnsFiltradas.filter((t) => t.estado === 'PAGADO').reduce((s, t) => s + t.monto, 0),
    [txnsFiltradas],
  );
  const reembolsos = useMemo(
    () =>
      txnsFiltradas
        .filter((t) => t.estado === 'REEMBOLSADO')
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-5">
        <KPICard
          title="GMV total"
          value={formatSoles(gmv)}
          subtitle={periodLabels[periodo]}
          icon={DollarSign}
          color="info"
        />
        <KPICard
          title="Revenue app 15%"
          value={formatSoles(revenue)}
          subtitle="Comisión plataforma"
          icon={TrendingUp}
          color="success"
        />
        <KPICard
          title="Pagado a agentes"
          value={formatSoles(totalAgentes)}
          subtitle="80% del GMV"
          icon={Users}
          color="default"
        />
        ...
        <KPICard
          title="Fondo seguro"
          value={formatSoles(FONDO_SEGURO_BALANCE)}
          subtitle="Balance actual"
          icon={Shield}
          color={FONDO_SEGURO_BALANCE < FONDO_SEGURO_ALERTA_MIN ? 'danger' : 'default'}
        />
      </div>
