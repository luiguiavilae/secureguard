...
import { type MockAgenteVerificado, type NivelScore } from '@/lib/mock-data';
...
interface Props {
  agentes: MockAgenteVerificado[];
}

export default function AgentesClient({ agentes }: Props) {
  ...
  const distritos = useMemo(
    () => ['TODOS', ...Array.from(new Set(agentes.map((a) => a.distrito).filter(Boolean))).sort()],
    [agentes],
  );
  ...
  }, [agentes, search, filterNivel, filterEstado, filterDistrito, sortKey, sortDir]);
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">
            {filtered.length} agente{filtered.length !== 1 ? 's' : ''} encontrados
          </p>
        </div>
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-100 bg-gray-50 text-xs font-semibold uppercase tracking-wide text-gray-500">
              ...
              <Th onClick={() => toggleSort('servicios_completados')}>
                Servicios <SortIcon k="servicios_completados" />
              </Th>
              <Th onClick={() => toggleSort('rating_avg')}>
                Rating <SortIcon k="rating_avg" />
              </Th>
            ...
            {filtered.length === 0 ? (
              <tr>
                <td colSpan={9} className="py-12 text-center text-sm text-gray-400">
                  No hay agentes con estos filtros
                </td>
              </tr>
            ) : (
              filtered.map((a) => (
                <tr
                  ...
                  onClick={() => {
                    window.location.href = `/agentes/${a.id}`;
                  }}
                >
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-xs font-semibold ${NIVEL_COLOR[a.nivel]}`}
                    >
                      {NIVEL_EMOJI[a.nivel]} {a.nivel}
                    </span>
                  </td>
                  <td className="px-4 py-3 tabular-nums text-gray-700">
                    {a.servicios_completados}
                  </td>
                  <td className="px-4 py-3">
                    <span
                      className={`font-semibold ${a.rating_avg >= 4.5 ? 'text-emerald-600' : a.rating_avg >= 4.0 ? 'text-yellow-600' : 'text-red-500'}`}
                    >
                  ...
                  <td className="px-4 py-3">
                    <div className="flex flex-wrap gap-0.5">
                      {a.badges.slice(0, 4).map((b) => (
                        <span key={b} title={b} className="text-base leading-none">
                          {EMOJI_MAP[b] ?? '🏅'}
                        </span>
                      ))}
                      {a.badges.length > 4 && (
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-medium ${ESTADO_COLOR[a.estado] ?? 'bg-gray-100 text-gray-500'}`}
                    >
                      {a.estado}
                    </span>
                  </td>
